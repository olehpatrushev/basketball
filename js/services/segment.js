...
export class SegmentService extends BaseService {
    setUpSegments(data = {}) {
        ...
        for (let i = 1; i <= 14; i++) {
            this.scene.getMeshByName('segment_' + i).setEnabled(false);
        }

        const segmentsRoot = this.scene.getNodeById("segmentsRoot");
        segmentsRoot.getChildren().forEach(child => this.scene.removeMesh(child));

        for (const segmentName in data) {
            const segmentData = data[segmentName];
            const segmentMesh = this.scene.getMeshByName(segmentName);
